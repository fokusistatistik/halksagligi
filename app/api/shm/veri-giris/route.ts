...
const veriGirisSchema = z.object({
  birim_id: z.coerce.number({ invalid_type_error: 'Geçerli bir birim seçiniz' }),
  personel_id: z.coerce.number({ invalid_type_error: 'Geçerli bir personel ID giriniz' }),
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (birimId) whereClause.birim_id = parseInt(birimId);
    if (personelId) whereClause.personel_id = parseInt(personelId);
